-- Vendored from https://github.com/lewis6991/fileline.nvim (MIT, Lewis Russell)
-- Goto filenames with line info, e.g. `nvim file.lua:10` or `file.lua(10:5)`.
local M = {}
...
local function reopen_and_gotoline(file_name, lnum, col)
    ...
    vim.schedule(function() vim.api.nvim_buf_delete(bufn, {}) end)

    lnum = math.min(math.max(1, lnum), vim.api.nvim_buf_line_count(0))
    vim.api.nvim_win_set_cursor(0, { lnum, col and col - 1 or 0 })

    if vim.fn.foldlevel(lnum) > 0 then vim.cmd.normal({ "zv", bang = true }) end
...
local function process_name(name)
    ...
    if not file_name then return name end

    suffix = suffix:gsub(":$", "")

    if suffix:match("^%d+$") then return file_name, tonumber(suffix) end
...
vim.api.nvim_create_autocmd("BufNewFile", {
    group = vim.api.nvim_create_augroup("personal_fileline", { clear = true }),
    nested = true,
    callback = function() M.gotoline() end,
})
